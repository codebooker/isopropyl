...
| Syslinux/Isolinux identity and patching | **Partial** | Exact release/custom-build conflicts are detected through descriptor-bound reads; unsafe names, overflow, read failure, oversized payload, or timeout fails closed. Independently pinned `6.03-2014-10-06`/`6.04-pre1` consumers reproduce ADV, extent/checksum patches, FAT32 VBR merge, and the exact Rufus Syslinux 6.02 MBR bootstrap. The descriptor-only mapper binds mirrored FATs/VBRs, FSInfo, `BPB_HiddSec`, the unique unaliased root file, and its exact fragmented chain. A witnessed staging policy binds base-ISO loader/config bytes, matching C32 and BIOS bundle roles, and exclusively materializes the complete unpatched root output with catalog/free-space accounting and final read-back. A new anonymous-regular-file transaction rebuilds the live map/plan, freezes exact write records, preserves final-sector slack, computes complete pre/post image hashes, and durably writes loader, backup VBR, primary VBR, then MBR activation last. It remaps and verifies every postimage plus the exact whole-image digest; errors return no result and poison only an unpublished anonymous file. Cross-boundary fragmented golden, stale/forged plan, short/zero/EINTR write, fsync, unrelated corruption, activation-order, cancellation, and slack vectors pass. The GUI supplies neither bundle and no block device is reachable. A production-owned FAT32 builder with proven unmount/loop-detach lifecycle, privileged streaming, QEMU/SeaBIOS, and physical certification remain. |
...
3. Complete the started Syslinux BIOS/dual-firmware profile by adding a
   production-owned FAT32 image builder with proven unmount/loop-detach lifecycle
   ahead of the implemented anonymous regular-file patch transaction. Then add
   bounded device streaming and pass QEMU/SeaBIOS/OVMF gates plus physical
   certification.
