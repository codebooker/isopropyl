...
3. Complete the started Syslinux BIOS/dual-firmware profile by adding bounded
   root `ldlinux.sys` materialization and the device transaction after the
   implemented config/C32 private-tree transform, then pass QEMU/SeaBIOS/OVMF
   gates and physical certification.
