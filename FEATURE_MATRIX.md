...
3. Complete the started Syslinux BIOS/dual-firmware profile by joining the
   implemented witnessed ISO-to-patched-image pipeline to one target/device
   authorization plan. Then add bounded privileged device streaming/read-back
   and pass QEMU/SeaBIOS/OVMF gates plus physical certification.
