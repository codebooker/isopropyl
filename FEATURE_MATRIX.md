...
| Raw SquashFS filesystem detection | **Planned** | ISOpropyl recognizes SquashFS-related ISO member layouts but does not yet identify a standalone `hsqs` filesystem at byte zero as Rufus does. Add descriptor-bound superblock parsing before using that evidence for recommendations. |
...
| Syslinux/Isolinux identity and patching | **Partial** | Exact release/custom-build conflicts are detected through descriptor-bound reads; unsafe names, overflow, read failure, oversized payload, or timeout fails closed. Independently pinned `6.03-2014-10-06`/`6.04-pre1` consumers reproduce ADV, extent/checksum patches, FAT32 VBR merge, and the exact Rufus Syslinux 6.02 MBR bootstrap. A witnessed staging policy binds base-ISO loader/config bytes, matching C32 and BIOS bundle roles, and exclusively materializes the complete unpatched root output with catalog/free-space accounting and final read-back. Matching pre/post-publication full-tree manifests mint an authenticated receipt; refreshed dataclass clones lose authority. A witnessed composite then binds that exact receipt, both bundles, config directory, root loader, and private FAT32 plan. The production-owned in-process builder turns the identity/hash-bound tree into a deterministic, preallocated MBR/FAT32 disk held only in an owner-only unlinked `O_TMPFILE`; it uses no loop, mount, formatter, pathname, or subprocess, and derives distinct nonreserved MBR/FAT IDs from output-relevant plan data. A separate parser revalidates mirrored FATs, FSInfo accounting, labels, dot entries, aliases/LFNs, allocation reachability, every file hash, and the whole-image identity. The descriptor-only transaction then rebuilds the live map/plan, freezes exact write records, preserves final-sector slack, and durably writes loader, backup VBR, primary VBR, then MBR activation last. The wrapper independently reparses and rehashes the live descriptor before enabling duplicate-descriptor streaming; any failure poisons the unpublished image. The composite exposes no builder/transaction injection and returns only a patched-attested owner. A separate non-executable target plan now binds the exact composite object and digests, a freshly re-probed complete removable-device observation and descendant topology, live block major:minor, exact image/capacity equality, fixed 512-byte logical sectors, fail-closed workspace/source non-residency, mandatory read-back, warnings, and a case-sensitive typed phrase through clone-losing process-local receipts. Geometry boundaries, root/nested configs, Unicode/LFN stress, stale/forged plans, confirmations, and receipts, short/zero/EINTR writes, source drift, metadata corruption, post-transaction gap mutation, descriptor reuse, fsync, activation order, cancellation, and slack vectors pass. The GUI supplies neither bundle and no privileged device executor is reachable. A root-owned transaction helper that distrusts serialized input, QEMU/SeaBIOS/OVMF, and physical certification remain. |
...
3. Complete the started Syslinux BIOS/dual-firmware profile by implementing the
   root-owned executor profile already required by the witnessed target plan.
   It must retain one exact target descriptor and lock through bounded streaming,
   durability, and read-back before QEMU/SeaBIOS/OVMF and physical certification.
