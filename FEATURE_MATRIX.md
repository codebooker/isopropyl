...
| Microsoft ISO downloader | **Partial** | The explicit-consent **Download Windows…** flow supports one exact public profile: Windows 11 25H2 v2 consumer multi-edition, English (United States), x64. The recommended browser-assisted path validates the generated Microsoft CDN capability's exact HTTPS host, path, canonical five-field query, and 5-minute-to-25-hour expiry without logging or persisting it. A separately selected direct resolver reads bounded Microsoft challenge bytes as inert text and fails closed on schema/service drift; it never executes Fido, PowerShell, JavaScript, or ISO contents and may be rejected by Microsoft. Both paths require the bundled exact filename/size/SHA-256 to match Microsoft's current official English x64 hash row, share the private resumable exact-range/full-rehash/no-overwrite transaction, and must inspect the result as an x64 Windows installer before loading it. Other releases, languages, architectures, Enterprise/Insider/account media, and automatic handling of Microsoft service drift remain unsupported. |
