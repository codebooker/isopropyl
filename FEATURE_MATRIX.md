...
| Syslinux/Isolinux identity and patching | **Partial** | Exact `6.03-2014-10-06`/`6.04-pre1` identities, matched C32/BIOS bundles, ADV/extent/checksum patches, FAT32 VBR merge, and the pinned Rufus Syslinux 6.02 MBR bootstrap are implemented. Authenticated staging feeds a deterministic, independently parsed MBR/FAT32 image held only in an unlinked `O_TMPFILE`; the descriptor-only patch transaction fsyncs and re-attests the entire result. Initial target authorization binds the composite, complete removable-device observation, source/workspace non-residency, exact capacity, 512-byte sectors, disk generation, and typed phrase before preparation. An optional fixed PolicyKit integration transfers the re-attested descriptor over authenticated `SOCK_SEQPACKET`; the root helper independently validates source, exact private-builder FAT geometry/FSInfo, pinned immutable Syslinux VBR code, sysfs topology, mounts/swapfiles, holders, removability, and `BLKGETDISKSEQ`, then uses PREPARED → COMMIT/CANCEL, one opened descriptor, stale GPT-header deactivation, MBR-last activation, cache invalidation, and mandatory full SHA-256 read-back. Post-activation cleanup is identity-guarded. The PolicyKit prompt explicitly describes the image data as caller-supplied. The Python helper remains backend-only and is not installed by ordinary packaging. A native hardened helper, installed PolicyKit/SCM_RIGHTS VM tests, QEMU/SeaBIOS/OVMF, hot-swap/failure tests, and physical certification gate GUI exposure. |
...
| Automatic GRUB/Syslinux downloads | **Partial** | The backend foundation uses a release-bundled v2 catalog that pins exact immutable upstream URLs, sizes, SHA-256 digests, purpose-specific bundle membership, licenses, and provenance for selected GRUB/Syslinux payloads. Preparation uses no-follow cache directories, detects parent-path replacement, is cancellable, progress-aware, deadline-bounded across connection/download/cache reads/binding, exact-match-only, and returns no partial set or mutable cache path. Only exact Syslinux identities have dependency mappings; the GRUB entries remain blank-media research inputs. The pure Syslinux patch consumer independently re-pins immutable bytes, but no normal write or GUI flow can invoke bootloader acquisition. |
...
| Use Windows CA 2023-signed installer bootloaders | **Planned** | Replacing the install media's bootloaders is separate from post-install `SkuSiPolicy.p7b` deployment and requires versioned extraction, signature/architecture validation, controlled file replacement, and firmware testing. |
...
| Exclusive target ownership | **Partial** | Every destructive child command fails fast behind a whole-device cooperative `flock`; `sfdisk` uses its native nonblocking lock. The backend-only Syslinux and GUI/CLI-reachable generic raw profiles add Linux block `O_EXCL`, one same-generation target descriptor, an end-to-end BSD lock, PREPARED → COMMIT/CANCEL linearization, durability, cache invalidation, and mandatory profile-specific read-back. A native helper/private namespace, broader transaction coverage, installed-integration VM hot-swap/failure tests, and physical certification remain; neither raw interface has an unsafe fallback. |
...
| Opt-in network use | **Policy** | Windows/Linux ISO, UEFI:NTFS, UEFI Shell, and boot-time-validation acquisition each require an explicit user action and final confirmation. Downloads are release-pinned and no remote executable or script is run on Linux; ordinary image writes never trigger these network paths. |
...
3. Certify and harden the backend-only Syslinux device transaction. The fixed
   ...
   and sector-zero-last activation are implemented. A native helper, installed
   PolicyKit/SCM_RIGHTS VM coverage, QEMU/SeaBIOS/OVMF, hot-swap/failure tests,
   and physical certification still gate GUI exposure.
