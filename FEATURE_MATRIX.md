...
|---|---:|---|
| Raw `.img`/DD writing | **Done** | The established GUI path has selection-to-consent identity binding including ctime, one no-follow source descriptor, target identity checks, cancellation, progress, optional byte read-back verification, and power-off. A replacement backend now snapshots an uncompressed regular source into a fully allocated anonymous `O_TMPFILE`, binds its exact plan/content to target topology, disk generation, capacity, 512-byte sectors, and typed confirmation, then transfers the one-shot descriptor through a separate authenticated PolicyKit protocol. The root transaction holds one exclusive target descriptor, durably deactivates a 1 MiB front guard plus source/physical tail sectors, mandates inactive bulk read-back, activates tail then front guard, optionally performs a complete final hash, and identity-gates emergency deactivation. GUI routing for every compressed, virtual, and VTSI raw source plus native/VM/physical certification remains; there is no broker-to-`dd` fallback. A validated image/target logical-sector mismatch—or a selected target with unknown sector size—leaves structured DD available only as an explicit warned choice. Plain MBR uses the conventional assumed 512-byte interpretation. |
...
| Exclusive target ownership | **Partial** | Every destructive child command fails fast behind a whole-device cooperative `flock`; `sfdisk` uses its native nonblocking lock. The backend-only Syslinux and generic raw profiles now add Linux block `O_EXCL`, one same-generation target descriptor, an end-to-end BSD lock, PREPARED → COMMIT/CANCEL linearization, durability, cache invalidation, and mandatory profile-specific read-back. GUI migration, a native helper/private namespace, broader transaction coverage, VM hot-swap/failure tests, and physical certification remain; no unsafe fallback will be exposed. |
...
1. Route every normalized raw GUI input through the authenticated device broker,
   certify its failure/hot-swap behavior, then extend privileged exclusive target
   ownership across every destructive transaction.
