...
| Exclusive target ownership | **Partial** | Every destructive child command fails fast behind a whole-device cooperative `flock`; `sfdisk` uses its native nonblocking lock. The backend-only Syslinux and GUI-reachable generic raw profiles add Linux block `O_EXCL`, one same-generation target descriptor, an end-to-end BSD lock, PREPARED → COMMIT/CANCEL linearization, durability, cache invalidation, and mandatory profile-specific read-back. A native helper/private namespace, broader transaction coverage, installed-integration VM hot-swap/failure tests, and physical certification remain; the GUI raw path has no unsafe fallback. |
...
1. Certify the unified GUI raw broker's installed PolicyKit/SCM_RIGHTS,
